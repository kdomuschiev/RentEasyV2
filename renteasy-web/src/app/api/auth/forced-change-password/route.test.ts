...
describe('POST /api/auth/forced-change-password', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 401 when jwt cookie is absent', async () => {
    ...
    const response = await POST(new Request('http://localhost:3000/api/auth/forced-change-password', {
      method: 'POST',
  ...
  it('sets new jwt cookie and returns role+accountState on success', async () => {
    ...
    expect(body).toEqual({ role: 'Tenant', accountState: 'Active' })
    expect(body.token).toBeUndefined()
    expect(mockFetch).toHaveBeenCalledWith(
      'http://localhost:5164/api/auth/forced-change-password',
